...
// Ana metrik PA (page_score); DS kullaniciya gosterilmez.
// mode='' : genel durum + son 7 gunun yeni yerlesimleri (PA ile)
// mode='top': tum zamanlarin en yuksek PA'li kaynaklari
function buildReport(domain, keyword, ov, rows, mode) {
  // Domain basina tek kayit: en yeni first_seen + en yuksek PA
  const byDomain = new Map();
  for (const r of rows) {
    const g = byDomain.get(r.source_domain);
    if (!g) {
      byDomain.set(r.source_domain, { domain: r.source_domain, pa: r.page_score, first: r.first_seen });
    } else {
  ...
  let baslik;
  if (mode === 'top') {
    baslik = '🏆 EN GÜÇLÜ KAYNAKLAR (PA)';
    listed = groups.slice().sort((a, b) => Number(b.pa) - Number(a.pa)).slice(0, 10);
  } else {
  ...
  if (listed.length) {
    ...
    for (const g of listed) {
      L.push(`• <b>${esc(g.domain)}</b> — PA ${esc(g.pa)} — ${esc((g.first || '').slice(8, 10) + '.' + (g.first || '').slice(5, 7))}`);
    }
  } else {
...
async function handleCommand(chatId, text) {
  ...
  // 'top' ozel mod; diger kelimeler anchor filtresi olur
